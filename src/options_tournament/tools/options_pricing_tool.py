...
class OptionsPricingTool(BaseTool):
    ...
    description = (
        "Compute theoretical option price and Greeks with Black-Scholes "
        "(spot, strike, expiry_days, volatility, option_type)."
    )
    parameters = {
        "type": "object",
        "properties": {
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        if err is not None:
            return json.dumps({"status": "error", "tool": "options_pricing", "error": err})
        ...
        nonfinite = any(
            k not in result or not math.isfinite(float(result[k]))
